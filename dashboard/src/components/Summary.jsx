  ...
  const marginUtilization =
    totalCapitalBasis > 0
      ? Math.min(
          ...
          Math.max(
            0,
            Math.round((deployedCapital / totalCapitalBasis) * 1000) / 10,
          ),
        )
      : 0;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-1 pb-1">
        <div className="space-y-1.5">
          ...
          <p className="text-xs sm:text-sm text-slate-400">
            Monitor live market movements, manage your portfolio, and start your
            trading journey.
          </p>
        </div>
      ...
      {!liveValuationComplete && liveUnavailableSymbols.length > 0 && (
        <div className="flex items-start gap-3 p-4 rounded-xl border border-amber-500/30 bg-amber-500/10 text-amber-300 text-xs leading-relaxed">
          ...
          <div>
            <span className="font-bold">Valuation Alert:</span> Live market
            quotes are currently unavailable for{" "}
        ...
        <Card className="hover:border-white/20 transition-all">
          ...
          <CardContent className="p-4 pt-0">
            ...
            <p className="text-[11px] text-slate-400 mt-1 tabular-nums">
              Locked in orders: ₹
              {reservedBalance.toLocaleString("en-IN", {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}
            </p>
          ...
          <CardHeader className="flex flex-row items-center justify-between p-5 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">
                Real-time breakdown of capital distribution across active
                portfolio holdings
              </p>
            </div>
            <Link to="/holdings">
          ...
          <CardContent className="p-5 pt-2">
            {liveAllocation.length === 0 ? (
              <div className="py-12 text-center text-xs text-slate-500">
                No active holdings. Build a position from your watchlist to view
                ...
                <div className="w-64 h-64 sm:w-72 sm:h-72 mx-auto relative flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none text-center p-4">
                    ...
                    <span className="text-xl font-black text-white font-mono tabular-nums leading-tight mt-1">
                      ₹
                      {liveCurrentValue.toLocaleString("en-IN", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </span>
                    <span className="text-xs text-cyan-400 font-semibold mt-1">
                      {liveAllocation.length} Instrument
                      {liveAllocation.length === 1 ? "" : "s"}
                    </span>
                  </div>
                </div>
          ...
          <CardContent className="p-5 pt-0 space-y-4">
            <div className="p-3.5 rounded-xl bg-white/[0.02] border border-white/5 space-y-2 text-xs">
              ...
              <div className="flex justify-between text-slate-400">
                ...
                <span className="font-semibold text-white tabular-nums">
                  ₹
                  {(balance + reservedBalance).toLocaleString("en-IN", {
                    minimumFractionDigits: 2,
                  })}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between text-[11px] pt-0.5">
                ...
                <div className="flex flex-col text-right">
                  ...
                  <span className="text-emerald-400 font-bold tabular-nums">
                    ₹
                    {totalAccountEquity.toLocaleString("en-IN", {
                      minimumFractionDigits: 2,
                    })}
                  </span>
                </div>
                ...
                {holdings.length === 0 ? (
                  <tr>
                    <td colSpan="7" className="py-8 text-center text-slate-500">
                      No active holdings. Place your first trade from the
                  ...
                  holdings.slice(0, 5).map((stock) => {
                    ...
                    const pnl = val !== null ? val - avgVal : null;
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {pnl !== null
                            ? `${isRowProfit ? "+" : ""}₹${pnl.toLocaleString(
                                "en-IN",
                                {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2,
