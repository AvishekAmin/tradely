...
export const ToastProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const mode = (
        lastOrderUpdate.mode ||
        lastOrderUpdate.side ||
        ""
      ).toUpperCase();
      const stock =
        lastOrderUpdate.name ||
        lastOrderUpdate.symbol ||
        lastOrderUpdate.instrument ||
        "";
        ...
        {toasts.map((toast) => (
          <div
            ...
            className={cn(
              ...
              toast.type === "success" &&
                "border-emerald-500/30 bg-[#0F1D17]/95 text-emerald-300 shadow-emerald-950/40",
              toast.type === "error" &&
                "border-rose-500/30 bg-[#1D0F12]/95 text-rose-300 shadow-rose-950/40",
              toast.type === "info" &&
                "border-cyan-500/30 bg-[#0F171D]/95 text-cyan-300 shadow-cyan-950/40",
            )}
          >
            {toast.type === "success" && (
