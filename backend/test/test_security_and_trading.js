...
const runAllTests = async () => {
  ...
  try {
    console.log("--- 1. Structured Logging & Sensitive Redaction ---");
    {
      ...
      assert(
        redacted.nested.secretKey === "[REDACTED]",
        "Sanitizes nested secret fields",
      );
      assert(
        redacted.nested.publicSymbol === "RELIANCE",
        "Preserves non-sensitive nested data",
      );
      assert(redacted.nested.qty === 10, "Preserves numerical fields");

      const bearerStr = sanitizeData(
        "Authorization: Bearer header.payload.signature",
      );
      assert(
        bearerStr.includes("Bearer [REDACTED]"),
        "Redacts Bearer tokens in raw string headers",
      );
    }
    ...
    console.log("\n--- 2. Secure Cookie Policy ---");
    {
      ...
      assert(
        defaultOpts.maxAge === 7 * 24 * 60 * 60 * 1000,
      ...
      assert(
        prodOpts.sameSite === "none",
        "COOKIE_SAME_SITE=none sets sameSite: 'none'",
      );
      assert(
        prodOpts.domain === "tradely.example.com",
        "COOKIE_DOMAIN sets cookie domain",
      );
    ...
    console.log("\n--- 3. Sliding-Window Rate Limiter ---");
    {
      ...
      assert(
        lastHeaders["RateLimit-Limit"] === 3,
        "Sets RateLimit-Limit header",
      );
      assert(
        lastHeaders["RateLimit-Remaining"] === 3,
        "Calculates RateLimit-Remaining header",
      );
      ...
      assert(
        lastHeaders["Retry-After"] >= 1,
        "Sets standard Retry-After header",
      );
      assert(
        lastJson.code === "TOO_MANY_REQUESTS",
    ...
    console.log("\n--- 4. Error Masking & Security Hardening ---");
    {
      ...
      assert(
        opJson.code === "INVALID_PAYLOAD",
        "Operational error keeps custom error code",
      );
      assert(
        opJson.message === "Custom business validation failure",
        "Operational error message is preserved",
      );
      ...
      const internalErr = new Error(
        "DB Connection string mongodb://root:secret@db:27017/prod leaked",
      );
      internalErr.stack =
        "Error: DB Connection string...\n    at connectDB (db.js:10:5)";
    ...
    console.log("\n--- 5. HTTP Health & Readiness Probes ---");
    {
      ...
      assert(
        healthData.service === "tradely-backend",
        "GET /health service identifier is 'tradely-backend'",
      );
      assert(
        typeof healthData.uptime === "number",
        "GET /health includes uptime metric",
      );

      const readyRes = await fetch(`${baseUrl}/ready`);
      assert(
        readyRes.status === 200,
        "GET /ready returns HTTP 200 when database connected",
      );
      const readyData = await readyRes.json();
      assert(readyData.status === "ready", "GET /ready status is 'ready'");
      assert(
        readyData.database.status === "connected",
        "GET /ready indicates database is connected",
      );

      const unauthNotFoundRes = await fetch(
        `${baseUrl}/non-existent-route-for-test`,
      );
      assert(
        unauthNotFoundRes.status === 401,
        "Unauthenticated access to undefined route is rejected with 401",
      );
    }
    ...
    console.log("\n--- 6. Phase 1-7 Regression & Data Isolation ---");
    {
      ...
      assert(
        Boolean(setCookieHeader) && setCookieHeader.includes("token="),
        "Signup response sets HttpOnly auth cookie",
      );

      const authCookie = setCookieHeader.split(";")[0];

      const authNotFoundRes = await fetch(
        `${baseUrl}/non-existent-route-for-test`,
        {
      ...
      assert(
        authNotFoundRes.status === 404,
        "Authenticated request to unknown route returns 404 Not Found",
      );

      const defaultWatchlist = await watchlistService.getWatchlist(userId);
      assert(
        Array.isArray(defaultWatchlist.symbols),
        "Creates default user watchlist",
      );
      assert(
        defaultWatchlist.symbols.length > 0,
        "Default watchlist has initial symbols",
      );

      const updatedWl = await watchlistService.addSymbol(userId, "SBIN");
      assert(
        updatedWl.symbols.includes("SBIN"),
        "Adds symbol to user watchlist",
      );
      ...
      assert(
        analytics.totalInvested === 15000,
        "Calculates total invested capital correctly (10 * 1500 = 15000)",
      );
      assert(
        analytics.currentValue > 0,
        "Calculates current holdings valuation based on live server quotes",
      );
      assert(
        analytics.valuationComplete === true,
        "Marks valuationComplete as true when instrument quotes are present",
      );
      assert(
        typeof analytics.unrealizedPnL === "number",
        "Computes unrealized P&L number",
      );
      ...
      const incompleteAnalytics =
        await analyticsService.getPortfolioAnalytics(userId);
      assert(
        incompleteAnalytics.valuationComplete === false,
        "Marks valuationComplete as false when quotes are missing",
      );
      assert(
        incompleteAnalytics.unavailableSymbols.includes(
          "UNLISTED_COMPANY_TEST",
        ),
        "Identifies unavailable symbols",
      );
      ...
      assert(
        Boolean(ocoResult.data.ocoGroup._id),
        "Creates dedicated OCO group",
      );
      ...
      const holdingAfterOco = await HoldingsModel.findOne({
        userId,
        name: "INFY",
      });
      assert(
        holdingAfterOco.reservedQty === 5,
        "Shared reservation locks exactly 5 shares for both legs",
      );

      const ocoGroup = await OcoGroupModel.findById(
        ocoResult.data.ocoGroup._id,
      );
      assert(ocoGroup.status === "ACTIVE", "OCO group status is ACTIVE");
      assert(
        ocoGroup.sharedReservation.amount === 5,
        "OCO group owns the shared 5-share reservation",
      );
